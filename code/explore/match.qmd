...
panel_univ$owner_any <- as.integer(panel_univ$houseown1 | panel_univ$houseown2)

m_pc <- matchit(owner_any ~ PC1 + PC2 + factor(ireg),
                data   = panel_univ,
                method = "nearest",
                distance = "mahalanobis")     # uses Mahalanobis on the PCs

matched_pc <- match.data(m_pc)
summary(m_pc)   # check balance on PC1, PC2, region
...
### Covariate balance after match on PCA 

```{r}

## covariate balance pre-post table

sum_pc <- summary(m_pc, standardize = TRUE, un = TRUE)

pre  <- as.data.frame(sum_pc$sum.all)    # before matching
post <- as.data.frame(sum_pc$sum.matched)


pre_df <- pre %>%
  tibble::rownames_to_column(var = "covariate") %>%
  dplyr::select(covariate, `Std. Mean Diff.`) %>%
  rename(smd = `Std. Mean Diff.`) %>%
  mutate(stage = "Unmatched")

post_df <- post %>%
  tibble::rownames_to_column(var = "covariate") %>%
  dplyr::select(covariate, `Std. Mean Diff.`) %>%
  rename(smd = `Std. Mean Diff.`) %>%
  mutate(stage = "Matched")



balance_df <- bind_rows(pre_df, post_df) %>%
  filter(
    covariate %in% c("PC1","PC2", paste0("factor(ireg)", 2:20))
  )


ggplot(balance_df, aes(x = smd, y = reorder(covariate, smd), color = stage, shape = stage)) +
  ...
  scale_x_continuous("Std. Mean Difference", labels = percent_format(accuracy = 1)) +
  labs(
    title    = "Covariate Balance: Before vs After Mahalanobis Matching",
    subtitle = "Matching on PC1, PC2, and region",
    y        = NULL,
    caption  = "Red dashed line = |SMD| = 0.1"
  ) +
...
### Fit reg on matched sample 

```{r}



# Get your matched data.frame
matched_pc <- match.data(m_pc)

# 2a) Simple difference in means via OLS
m1 <- lm(hh_univ ~ owner_any, data = matched_pc)
coeftest(m1, vcov = vcovHC(m1, "HC1"))

# 2b) Adjusted regression (still unbiased if match is good)
m2 <- lm(hh_univ ~ owner_any + PC1 + PC2 + factor(ireg),
         data = matched_pc)
...
### Quantile reg on matched sample
